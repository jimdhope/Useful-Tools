...
export function statusKind(status: string): StatusKind {
  ...
  // Everything else is "live" / unplanned
  return "live";
}

export function statusLabel(status: string): string {
  // Check specific DNO statuses FIRST
  const npgStatuses: Record<string, string> = {
    response_not_started: "Response Not Started",
    fault_logged: "Fault Logged",
    dispatching: "Dispatching Resources",
    engineers_en_route: "Engineers En Route",
    on_site_investigation: "On-Site Investigation",
    restoration_in_progress: "Restoration In Progress",
    validation_checks: "Validation / Safety Checks",
  ...
  const ssenStatuses: Record<string, string> = {
    ...
    engineer_en_route: "Engineer En Route",
    engineer_on_site: "Engineer On Site",
    restored: "Restored",
    unplanned: "Unplanned",
    fixed: "Fixed",
...
export function statusColor(status: string): string {
  ...
  // Northern Powergrid colors
  const npgColors: Record<string, string> = {
    response_not_started: "#8892a6",
    fault_logged: "#ffa726",
    dispatching: "#f59e0b",
    engineers_en_route: "#42a5f5",
    on_site_investigation: "#ab47bc",
    restoration_in_progress: "#26c6da",
    validation_checks: "#66bb6a",
    fully_resolved: "#22c55e",
  };
  if (npgColors[status]) return npgColors[status];
  
  // Generic colors for other DNO statuses
