...
export default function PowerCutMap() {
  ...
  // Get coordinates for an incident, falling back to postcode geocoding
  ...
  return (
    <div className="h-full w-full flex flex-col">
      <header className="flex shrink-0 flex-wrap items-center gap-3 border-b border-border bg-card px-4 py-2 md:flex-nowrap md:gap-4 md:px-6">
        ...
        <div className="relative min-h-0 flex-1 min-w-0">
          ...
          {!showLegend && (
            <button
              ...
              className="absolute right-3 top-3 z-[1000] rounded-lg border border-border bg-card px-3 py-1.5 text-xs text-muted-foreground"
            >
              Show legend
            </button>
          )}
...
function KpiBadge({
  ...
  return (
    <div className="flex min-w-[70px] flex-col items-start rounded border border-border bg-muted px-2 py-1">
      <span className="text-[0.65rem] uppercase tracking-wider text-muted-foreground">{label}</span>
      <span className={`text-base font-bold leading-tight ${className === "live" ? "text-destructive" : className === "planned" ? "text-amber-500" : "text-primary"}`}>
        {value}
      </span>
    </div>
  );
